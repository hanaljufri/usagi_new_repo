...
public class UsagiMain implements ActionListener {
	...
	public UsagiMain(String[] args) {
		...
		if (args.length == 1) {
			...
			Global.folder = new File("\\Users\\Hana\\Downloads\\usagi_repo\\dist\\vocab").getAbsolutePath();
		}
...
//		setup the index if index is not setup
//		instantiate search and index related classes
//		receive input from command line (search term)
//		pass input to UsagiSearchEngine search()
//		and return ArrayList of scoredConcepts
//		Print the results using toString() of the object
...
//		frame.setVisible(true);
		...
		BufferedReader reader = new BufferedReader(
	     ...
	     try {
			...
			System.out.print("With Filters? (Y/N) ");
			String answer = reader.readLine();
			if (answer.contentEquals("y")||answer.contentEquals("Y")) {
				System.out.print("Enter a vocabulary: ");
				String vocabInput = reader.readLine();
				if (!vocabInput.isEmpty()) {
					vocab.add(vocabInput);
				}
				System.out.print("Enter a domain: ");
				String domainInput = reader.readLine();
				if (!domainInput.isEmpty()) {
					domain.add(domainInput);
				}
			}
			//build not using build file, use the usual javac
			
			List<ScoredConcept> results = Global.usagiSearchEngine.search(input,true,null,domain,null,vocab,true,true);
